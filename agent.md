...
- V3.8.1「Trip 刪除墓碑與唯一識別」資料方案、靜默切換與兩筆種子 Trip 保護介面已於 2026-09-10 確認；候選程式、本機隔離 baseline、17 項 SQL、rollback／重套與 GitHub Actions 隔離 API 均已完成。2026-09-11 已建立 production logical backup、套用 migration `20260910140949`，migration history 完全對齊且 production-safe 唯讀 postflight 10 項全數通過；桌面、390×844、即時離線／重連與兩個獨立 origin 的雙分頁驗證已完成，雙分頁驗證發現的編輯 modal 殘留已修正並完成回歸。Product Owner 已核准更新提示文案並授權完整發布，V3.8.1 前端資產、metadata、`main`、annotated tag 與 GitHub Pages 已完成；Android／iOS 實體裝置、PWA 冷啟動離線及舊版實機必要更新提示列為發布後補驗。正式 metadata 為 `minimumSupportedVersion: 3.8.1`、`forceUpdate: true`。`free-travel-2026-01` 與 `group-tour-2026-10` 可編輯但前端／資料庫均不得刪除；目前 Trip 被刪除後不顯示提示，依既有規則切換至預設 Trip 的每日詳細行程及適合 Day。

...
- V3.5.4「管理者帳號名稱映射與新 Trip ID」已於 2026-08-27 合併、建立 `v3.5.4` tag 並部署；採非強制更新。正式 migration、資料庫角色回歸、Super Admin 介面驗收、離線新增阻擋、本機重複提示與正式站版本更新提示均已通過。V3.5.5「行程返回、外部連結與資訊文字醒目」已由 Product Owner 完成合併與部署；電腦版驗證通過，手機實機驗證延後。V3.5.6「版本更新最低支援版本機制與手機文字選色工具列修正」已完成程式實作與發布前驗證；時間修正包含同一天離開時間不得早於到達時間，以及錯誤範例只使用半形 `08:00`；待手動合併、建立 tag 與部署。其後為 V3.6.0 地點間預估移動資訊。iOS standalone PWA Google OAuth 仍保留日後補驗證。

...
- V3.6.1「Android 記帳金額欄位與 PWA 更新流程修正」已於 2026-09-01 合併至 `main`、建立 `v3.6.1` tag 並部署。Android 手機已驗證更新提示一次點擊即可完成，記帳金額輸入欄位不再變形或超出畫面；更新後介面顯示偏慢已完成初步量測，兩項改善經 Product Owner 核准並排入 V3.6.5，尚未實作。
- V3.6.3「V3.6.2 發布後前端修正」已於 2026-09-03 合併至 `main`、建立 `v3.6.3` tag 並部署；採一般更新，正式 metadata 與主程式資產已驗證。iOS／Android 實機及登入後管理者流程保留發布後補驗。

...
assert.match(itinerary, /其他資訊分類捷徑/);
assert.match(itinerary, /otherInfoFolders\.map/);
assert.doesNotMatch(itinerary, /folder\.parentId !== null/);
assert.match(workspace, /consumeExternalReturnDay/);
assert.match(workspace, /getExternalReturnTripId/);
assert.match(workspace, /cloudUpdatedAt:\s*currentStoredRecord\?\.cloudUpdatedAt/);

...
assert.match(page, /COMMONS_CANDIDATE_PAGE_SIZE/);
assert.match(page, /COMMONS_CANDIDATE_PAGE_SIZE = 6/);
assert.match(page, /找不到符合條件的照片，請調整搜尋詞後再試。/);
assert.match(page, /這一批沒有新的照片；可再換一批或調整搜尋詞。/);
assert.match(page, /已沒有更多照片；可調整搜尋詞或改用其他來源。/);
assert.match(page, /setCommonsNextOffset\(offset === 0 \? null : nextOffset\)/);

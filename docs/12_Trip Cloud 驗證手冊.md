...
## V3.5.0 敏感資訊卡片權限驗證（Android／Chrome PWA 已通過；iOS 實機列發布後）
...
4. 於另一台管理者裝置新增其他資訊，確認原裝置手動重新載入後可取得資料；跨裝置「自動即時刷新」另列 BUG026，納入 V3.5.1。
...
4. migration 與角色回歸已完成，V3.5.0 已發布；PWA 舊版升級流程列為發布後補驗證。

### C. 發布後補驗證（Android／Chrome PWA）

1. ✅ 已通過（2026-08-26）：以 V3.4.11 作為基準升級至 V3.5.0，強制更新只需按一次且更新後不是空白頁；更新後已儲存資料仍在，未儲存表單警語正常。
2. ✅ 第 2 項已通過（2026-08-26）：Android／Chrome PWA 啟動與版面。
   - Android 維持 V3.4.11 的單次原生 Splash 圖示，不得新增第二張圖示或改變載入順序。
   - Android 管理／編輯欄位聚焦與關閉後，畫面維持正常寬度，不左右溢出；一般縮放能力仍保留。
   - 電腦 PWA 啟動時可短暫顯示米白底並正常進入 App，不得持續停在空白頁。

> App 分享功能僅完成規格、模擬圖與修改邊界確認，未納入 V3.5.0 程式；實作與實機驗證移至 V3.5.1。

> 發布後跨裝置回歸另發現：其他資訊、領隊／導遊資料及共同準備清單雲端寫入成功，但同一 Trip 的另一台裝置不會即時刷新；切換行程資料庫或重新取得 App 焦點後才載入最新資料。此問題不影響既有寫入與手動重新載入，並需一併驗證不同帳號同時修改同一筆資料時的更新競態，列入 V3.5.1 修正，不回溯修改已發布的 V3.5.0。

> V3.5.1 共同清單衝突驗收：不同 ID 採聯集；離線／結構修改依修改前快照、本機結果與最新雲端結果三方合併；同一 ID 的同欄位或勾選狀態同時修改，以最後成功雲端寫入結果為準，不顯示衝突 UI。Realtime 事件只負責通知並重新讀取權威雲端資料，不得清除或覆蓋尚未送出的本機 pending。

### D. 發布後補驗證（iOS；無設備，保留未驗證）

> 2026-08-26 因目前沒有 iOS 實機設備，本大項暫時跳過；不標記通過或失敗，未來取得設備後再依下列項目補驗證。
...
### E. 第 4 項：V3.5.0 發布後驗證結案（已通過）

✅ 2026-08-26 經 Product Owner 確認結案：強制更新與 Android／Chrome PWA 已通過；iOS 因無設備保留未驗證；BUG026 跨裝置即時刷新／多帳號競態及 App 分享移入 V3.5.1，不再作為 V3.5.0 驗證阻擋項目。

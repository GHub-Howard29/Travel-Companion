...
2. 從 V3.4.11 開啟候選版時，更新提示不可關閉或選擇「稍後更新」。
3. 更新提示需說明本次敏感資訊 RLS、同步修正與權限修正；已儲存資料不被清除，未儲存表單需先儲存。
4. migration、角色回歸與 PWA 更新完成後，才能將 V3.5.0 標示為已發布。

### C. 發布前 Android／Chrome PWA 實機驗收（不含 iOS）

1. Android 維持 V3.4.11 的單次原生 Splash 圖示，不得新增第二張圖示或改變載入順序。
2. Android 管理／編輯欄位聚焦與關閉後，畫面維持正常寬度，不左右溢出；一般縮放能力仍保留。
3. Android／Chrome 回歸其他資訊、領隊／導遊同步、手動重試、角色權限及分享入口。
4. 從 V3.4.11 升級候選 V3.5.0，確認強制更新只需按一次且更新後不是空白頁，更新後已儲存資料仍在，未儲存表單警語正常。

...
2. ⏳ 第 2 項（待驗證）：Android／Chrome PWA 啟動與版面。
...
> 發布後跨裝置回歸另發現：其他資訊、領隊／導遊資料及共同準備清單雲端寫入成功，但同一 Trip 的另一台裝置不會即時刷新；切換行程資料庫或重新取得 App 焦點後才載入最新資料。此問題不影響既有寫入與手動重新載入，列入 V3.5.1 修正，不回溯修改已發布的 V3.5.0。

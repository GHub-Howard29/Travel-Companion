# Bug 修正紀錄

> 本文件只追蹤未修正、待補驗與近期修正。
>
> 重整前的完整 BUG001～BUG031 內容原檔保留於《[archive/bug-history/03_Bug修正紀錄_完整歷史](archive/bug-history/03_Bug修正紀錄_完整歷史.md)》，沒有刪除。
>
> 最後整理：2026-09-26

## 未修正

### BUG007：帳本排序方式

- 現況：帳本依建立時間遞增排序，最新資料顯示在最下方。
- 狀態：待 Product Owner 確認預期排序與對現有瀏覽習慣的影響後再實作。

### BUG032：離線前未開啟 Trip 時照片未預載

- 問題：Android PWA 在線時若沒有逐一點開各 Trip／行程卡片，切換離線後仍可看到文字內容，但該 Trip 的照片不會出現。
- 實機觀察：2026-09-26 驗證離線冷啟動時確認，照片必須先進入該 Trip 觸發載入後才會進入可離線使用狀態。
- 影響：目前「離線可用」實際只保證已載入過的文字／照片資源；使用者若未預先開啟每個 Trip，離線時內容不完整。
- 狀態：待釐清目前照片快取策略與預載範圍，再決定修正方式；Android 實機已穩定重現。

### BUG033：個人帳本顯示共用帳本代記文案

- 問題：沒有共用帳本權限的使用者進入個人帳本時，仍會看到「已依登入 Email 預設，可代其他成員記帳」。
- 預期：此文案只應在具有共用帳本／代其他成員記帳能力的情境出現；個人帳本作業不應顯示。
- 狀態：待修正顯示條件並回歸個人帳本與共用帳本兩種權限情境。

### BUG012：多人帳本 Realtime 即時同步未生效

- 問題：A、B 兩個設備同時停留在同一 Trip 帳本頁時，A 新增或刪除帳目後，B 不會透過 Realtime 立即看到變動。
- 實機觀察：2026-09-26 Android／跨裝置回歸確認新增與刪除皆可重現；既有 30 秒輪詢備援正常，B 最遲可在約 30 秒內自動取得變動，不需要切換功能頁面。
- 影響：資料最終會同步，沒有確認到資料遺失；問題限縮為 Realtime 即時更新路徑未生效，造成多人同時記帳時畫面延遲。
- 狀態：待重新檢查 Realtime subscription 生命週期、事件過濾與收到事件後的帳本 state 刷新；30 秒輪詢備援維持現況，不視為故障。

## 暫緩補驗

依 Product Owner 指示，iOS 相關實機補驗目前排除，不列入近期消項目標。

| 編號 | 主題 | 剩餘範圍 |
|---|---|---|
| BUG003 | 手機附件拍照／相簿／檔案選擇 | Android 已通過；僅剩 iOS |
| BUG009 | iOS 安裝 PWA 引導 | iOS Safari 與加入主畫面 |
| BUG010 | iOS 照片同步失敗 | iOS IndexedDB、Canvas、Storage 與背景切換 |
| BUG011 | Android 附件拍照入口 | Android 已通過；僅剩 iOS |
| BUG015 | Android PWA／iOS Safari 更新提示 | Android 已通過；僅剩 iOS Safari |

已完成驗證與舊修正紀錄移至《[archive/bug-history/03_Bug修正紀錄_完整歷史](archive/bug-history/03_Bug修正紀錄_完整歷史.md)》與版本更新紀錄保存。

## Bug 管理原則

- 新 Bug 先記錄問題、重現條件、影響、根因假設、修正與驗證結果。
- 已修正但仍有實機或風險補驗的項目，保留於本文件。
- 發布且驗證完整後，將詳細記錄搬入 `archive/bug-history/`，本文件不長期累積已結案流水帳。
- 歷史紀錄只搬檔、不刪除；本文件與《09_待辦事項_TODO》的未完成狀態必須一致。

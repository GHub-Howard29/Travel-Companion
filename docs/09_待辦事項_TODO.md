...
- [ ] V3.9.1 第一優先：Commons 實體導向候選照片精準篩選規格已核准並取得分階段實作授權；精確硬篩／評分／排序／終態純邏輯，以及 Wikidata 唯一 QID、直接 P31／P18／P373、Commons 直接 Category／P180 `depicts` 的離線回應解析、跨途徑去重與順序作業協調器已完成自動契約測試，尚未接入 Edge Function 正式流量。協調器已限制單一進行中請求、3 秒單次 timeout、9 次／20 秒／30 張硬停與 continuation 同層優先。真實上游取得、短效 token／共享快取與完整 UI 尚未實作。影像辨識維持排除；migration 細節與真實矩陣／spike 仍須個別核准。詳見《50》。

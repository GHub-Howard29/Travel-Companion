...
- [ ] V3.9.1 第一優先：Commons 實體導向候選照片精準篩選規格已核准並取得分階段實作授權；精確硬篩／評分／排序／終態純邏輯、Wikidata／Commons 離線回應解析與順序作業協調器，以及第五批固定請求參數、Commons `imageinfo` 中介資料純文字清理／來源限制／候選證據組裝已完成自動契約測試，尚未接入 Edge Function 正式流量。協調器已限制單一進行中請求、3 秒單次 timeout、9 次／20 秒／30 張硬停與 continuation 同層優先；文字搜尋只接受單一已採用詞，直接 Category 不遞迴，P18／多個直接 Category／P180 證據可合併後交由既有硬篩與排序。真實上游取得、短效 token／共享快取與完整 UI 尚未實作。影像辨識維持排除；migration 細節與真實矩陣／spike 仍須個別核准。詳見《50》。

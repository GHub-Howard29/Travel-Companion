...
> 最後更新：2026-09-10
>
> 已發布 App 版本：V3.7.4；Git 版本標籤 `v3.7.4`、GitHub Pages 與正式 `travel-route` Edge Function 部署已完成。
...
- [x] V3.7.4：每日行程地圖地點搜尋修正已於 2026-09-10 發布；正式授權搜尋「金閣寺」回應 HTTP 200 並取得 5 筆候選，未登入請求回應 HTTP 403。採一般更新，未變更資料庫或既有行程資料。
- [ ] V3.8.0：每日行程卡片抬頭依設定地點顯示景觀／地點照片；非景觀地點的地圖或地點照片來源、授權標示、快取與降級策略須於實作前完成確認。
- [ ] V3.9.0：使用者使用紀錄與開發者二次驗證；設計已於 2026-09-10 確認，待 V3.8.0 完成後實作。入口只向 `haw1971@gmail.com` 的獨立 `system_developer` 授權顯示，進入時每次重新驗證 TOTP，後端仍以私有 schema、RPC 權限、身分綁定、`aal2` 與短效 TOTP 多層阻擋未授權存取。完整規格見 `docs/36_V3.9.0_使用者使用紀錄與開發者二次驗證規格.md`。
...
- [x] V3.6.2：行程天數縮短刪除與歷史行程鎖定（開發、migration、驗證、Git 版本標籤與手動部署已完成；發布狀態為「已發布」）。
  ...
  - [x] 發布後實測確認縮短行程天數、台灣日期邊界、歷史 `trip_editor`／管理者、私人與既有規則、離線與同步均通過。
  ...
  - [x] 發布後由舊版進入 V3.6.2 時，更新提示正確顯示「必要更新」。
  - [x] BUG029 已轉列 V3.6.3：更新完成後的版本資訊把 V3.6.2 錯誤顯示為「一般更新」；V3.6.2 實際必要更新提示與更新流程已驗證通過。
  - [x] 桌面 Chrome、桌面 Edge、桌面 PWA、Android Chrome、Android PWA、縮短確認視窗及歷史警示版面均通過。
  - [ ] 補驗 iOS Safari 與 iOS standalone PWA。
- [x] V3.6.3：V3.6.2 發布後前端修正（已合併、建立 `v3.6.3` 標籤並部署，採一般更新）。
  - [x] 納入 BUG029 初步分析：`VersionInfoModal` 把 `isMandatoryForCurrentClient` 同時當成版本發布政策，導致升級完成後錯顯示「一般更新」。
  - [x] 彙整 V3.6.2 縮短天數、歷史角色、離線／同步及裝置實測結果；iOS Safari／standalone PWA 明確保留補驗。
  - [x] 分離「客戶端目前是否必須更新」與「版本本身的發布政策」資料來源；更新提示維持既有安全阻擋，版本資訊／歷史顯示固定政策。
  - [x] 補齊 V3.6.2 更新後版本資訊、歷史版本必要／一般更新標示與更新提示資料來源分離的自動驗證。
...
- [ ] V3.6.4：歷史唯讀參與者與跨裝置資料變動提醒（已發布；只待 iOS、Android 與兩台實體裝置驗證）。
  ...
  - [x] 確認資料變動提醒的事件範圍、UI 文案、刷新粒度、未儲存狀態、離線／漏接備援及同裝置事件排除。
  - [x] 確認修訂訊號只供仍具 `trip_editor` 指派或 `super_admin` 的已登入帳號讀取，payload 僅含最小修訂資訊，且不得作為授權依據。
...
- [x] V3.7.0：當日後續行程時間連動調整（2026-09-08 已完成 `v3.7.0`、GitHub Pages 部署與正式 metadata／資產確認，採一般更新）。
  ...
  - [x] 實作錯誤聚焦狀態；沿用既有 `focusAndRevealControl`，不改動 iOS viewport／縮放修正。
...
- [x] V3.7.2：農曆日期與 Day 語意配色（2026-09-08 已完成 `v3.7.2`、GitHub Pages 部署與正式 metadata／資產確認，採一般更新）。
  - [x] Day 標題以離線農曆換算顯示干支年、月與日，與國曆使用相同字體與顏色。
  - [x] Day 按鈕採淺藍／淺綠／淺紅背景與同色字體，最後一天優先；選取時維持淺色語意配色。
  - [x] 新環境可使用 `npm run setup` 依 lockfile 安裝全部預設套件。

...
- [ ] V3.5.5：行程返回、外部連結與資訊文字醒目（候選實作完成、待實機驗證；metadata 已升為 `3.5.5`，尚未發布）。
  - [x] 刪除目前行程後，返回首頁改為直接套用與進入 App 時相同的 `findDefaultTrip` 選取規則。
  - [x] 資訊卡片 HTTP／HTTPS 超連結改由系統外部處理；Google Maps universal URL 可優先交由已安裝 App 處理，否則由外部瀏覽器回退。
  - [x] 資訊卡片內容及每日詳細行程的「說明」欄已支援選取單行或局部文字並設定指定顏色；不提供整欄統一顏色。實作只修改這兩個文字編輯欄位，其他 App 欄位、圖示與功能未變動。
  - [ ] Android、iOS 與桌面 PWA 實機驗證：刪除後選取、一般外部連結、Google Maps App／瀏覽器回退、手機長按與電腦拖曳局部套色，以及儲存／重載／離線同步／角色讀取。

...
> 最後更新：2026-08-30
>
> 已發布 App 版本：V3.6.1；V3.5.2 資料庫維護已完成且無獨立 App 發布。
...
- [ ] V3.6.0：地點間預估移動資訊已於 2026-08-30 發布；完整角色矩陣、正式站 OAuth 自動化登入與 Android／iOS 實機仍待發布後補驗證。
  ...
  - [x] 候選程式 `3e89cbc` 已合併至 `main`（`0cce637`），建立 `v3.6.0` tag 並部署；正式 `app-version.json`、V3.6.0 Service Worker 接管、版本顯示、Guest 行程及已存路線均已通過。
...
- [ ] V3.6.1：Android 記帳金額欄位與 PWA 更新流程修正已於 2026-09-01 發布；保留跨平台補驗證與更新效能檢查。
  ...
  - [x] 2026-09-01 已完成合併、建立 `v3.6.1` tag、部署與正式版本文件更新。
  - [ ] 量測有更新時的新版下載、Service Worker 接管、reload 與 App 初始化耗時，定位使用者介面重新顯示體感偏慢的階段後再決定是否修正。
- [ ] V3.6.2：行程天數縮短刪除與歷史行程鎖定（開發、migration、驗證與必要更新候選設定已完成，等待 Product Owner 手動部署後回報）。
  ...
  - [x] Product Owner 確認 V3.6.2 採必要更新；候選 metadata 已設定 `minimumSupportedVersion: 3.6.2`，並保留 `forceUpdate: true` 供舊版 App 橋接。
  ...
  - [ ] 覆驗縮短 1 天、縮短多天、移除空白 Day、移除有卡片／路線 Day、取消確認、增加天數、歷史角色矩陣、離線與同步失敗。
  - [x] lint、build、V3.6.2 純函式、migration 靜態驗證及已連結資料庫角色 SQL 已通過。
  - [x] App 版本、公開 metadata、版本歷史與發布說明已完成 V3.6.2 必要更新候選設定，可供 Product Owner 手動部署。
  - [ ] Product Owner 手動部署完成後回報，再更新正式發布紀錄、Git tag 與發布後桌面／Android／iOS PWA 驗證狀態。

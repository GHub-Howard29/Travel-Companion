...
> 目前開發版本：V3.5.0（需求已確認，尚未開始）；已發布版本：V3.4.1。
>
...
- [ ] 修正共同清單離線管理時刪除項目造成分類排序重置：A 將「其他」分類移至最上方後離線刪除其中一筆，分類不得自動移至最下方；A 恢復連線後也不得以錯誤排序覆蓋 B 原有分類位置。
- [ ] 實機驗證共同／私人清單項目編輯：點擊鉛筆後僅在原項目文字列內編輯；Enter 或離開欄位會儲存、Esc 或取消按鈕會取消，編輯中均顯示儲存／取消按鈕；共同清單不顯示額外的編輯表單或修改分類。
- [ ] 實機驗證共同清單新增分類：分類欄位可下拉選擇既有分類，也可輸入新分類；未填分類不得新增，且不再自動帶入「其他」。
- [ ] 實機驗證每日行程離線限制：斷網時不顯示「管理」入口；若管理中斷網，表單與管理模式會立即關閉，既有行程資料不變。
- [ ] 實機驗證每日行程儲存後維持目前 Day：在 Day 2 以上新增、編輯或刪除卡片後，畫面不得跳回 Day 1。
- [ ] 實機驗證每日行程到達／離開時間：卡片顯示「到達 → 離開」；無離開時間時自動帶入到達時間；僅填離開時間時自動帶入到達時間；無到達時間的卡片仍置於當日最下方。
- [ ] 實機驗證恢復連線提示框：提示框以海水藍框線與背景清楚區隔，既有「稍後處理／重新載入最新資料」流程與 6 秒提醒行為維持不變。

...
> 目前開發版本：V3.4.1（清單離線 UI 與刪除同步補正後待實機驗證）；已發布版本：V3.4.0。
>
> 已發布版本：V3.4.0。
>
> 最後更新：2026-08-19
...
目前有效的功能快照、待改善與待驗證清單，請以 `docs/14_專案現況總覽.md` 為優先來源；本文件保留較完整的待辦與歷史規劃。
...
## V3.3.3 確認清單同步與分類排序（已發布）

- [x] 私人確認清單進入頁面或切回 App 時，會比較本機與雲端最後更新時間。
- [x] 雲端資料較新時下載覆蓋本機；本機資料較新時才同步更新雲端。
- [x] 私人確認清單的新增、修改、排序與刪除均會更新雲端清單時間。
- [x] 共同確認清單分類根目錄支援拖拉與上下按鈕排序，並會帶著全部子項目移動。
- [x] App／PWA 更新提示切換至 V3.3.3。
- [x] `npm run lint` 通過。
- [x] 已重新安裝 lockfile 指定依賴，`npm run dev` 可正常載入本機 App。
- [x] Product Owner 本機回歸：同一管理者帳號的兩台設備交替修改私人確認清單，較新的內容會同步保留。
- [x] Product Owner 本機回歸：共同確認清單分類排序後，子項目會一併移動，重新整理後仍維持正確順序。
- [x] Product Owner 手動推送、合併並執行 V3.3.3 發布／部署流程。

版本控制：

- V3.3.3 在手動推送、合併與發布完成前，不啟動 V3.4.0。
...
- [x] 共同檢查清單、私人確認清單、領隊／導遊聯絡資訊與其他資訊的管理模式可調整卡片順序。
- [x] 私人確認清單已勾選項目自動置底。
- [x] `npm run build` 通過。
- [ ] Product Owner 回歸：旅程開始日、中間日、結束日與非旅程期間的預設 Day。
- [ ] Product Owner 回歸：調整各資訊頁卡片順序後儲存、重新整理與重新進入頁面。
- [ ] Product Owner 回歸：私人確認清單勾選／取消勾選後，已勾選項目會自動置底。
...
## V3.4.0 前端載入效能優化（已部署，待實機驗證）

啟動條件：

- [x] V3.3.0 外幣換算主功能回歸全部通過。
- [x] V3.3.0 無已知需改善問題。
- [x] V3.3.3 已由 Product Owner 手動正式發布與部署。
...
- [x] Product Owner 完成 V3.4.0 發布、合併及部署。
- [ ] 完成部署後手機實機驗證：PWA 安裝／更新、首次開啟、離線快取、各工具切換與 Excel 匯出。

---

## V3.4.1 離線狀態、Other Info 同步與版本提示修正（開發中）
...
- [x] 分離「發現新版本」與「已更新至最新版」兩種提示模式。
- [x] 修正目前版本與可更新版本的顯示語意，已讀版本不再冒充目前執行版本。
- [x] 修正 Android／iOS 手機對話框高度、捲動與底部安全區。
- [ ] 實機驗證「馬上更新」、「稍後更新」、「我知道了」及手機按鈕位置。
- [x] 安裝旅行助手提示改為登入且 App 載入完成後才顯示；保留啟動時監聽原生安裝事件，已安裝／standalone 模式不顯示。
- [x] 沒有原生安裝事件時直接顯示既有手動安裝說明；「稍後」隱藏 7 天，永久關閉與本次瀏覽取消安裝分開記錄。
- [ ] 實機驗證 Android 原生安裝、iOS 分享加入主畫面、無安裝事件手動說明、7 天稍後冷卻及已安裝不再顯示。
- [x] 修正 PWA 斷網重開可用、但離線刷新會顯示「無法顯示頁面」的導覽 fallback／快取時序問題。
- [x] 實機驗證離線重開、離線刷新及刷新後旅行工具切換均正常；離線刷新約等待 30 秒後載入既有本機資料，顯示正常。
...
- [x] 實機驗證離線側欄顯示目前使用者代號與 Email；無可確認 session 時不顯示舊帳號。
- [x] 修正共用清單離線時仍先嘗試同步雲端、失敗後才顯示本機資料的問題。
- [x] 登入並取得私人清單同步權限後，立即背景預載今天與未來行程的私人清單至帳號隔離的本機快取；歷史行程不背景預載。
- [x] 實機驗證登入完成後未先開啟私人清單，直接離線仍可顯示該帳號已預載的私人清單；離線期間不發出 Supabase 請求。
- [x] 修正離線排序於恢復連線後被舊雲端順序覆蓋：共用／私人清單均改用帳號隔離的持久化 pending revision，成功同步後才清除。
- [x] 共用清單離線勾選同樣納入 pending revision，避免重連時被舊雲端勾選狀態覆蓋。
- [x] 實機重驗共用與私人清單：離線排序後關閉／重開 App、恢復連線及跨裝置確認順序不回退；共用清單離線勾選亦不回退。
- [x] 修正私人清單管理將「查詢失敗」誤判為「未有私人歷史紀錄」；以實際查詢狀態顯示載入中、查詢失敗或成功無資料，不只依 `isOnline` 判斷。
- [x] 私人歷史來源查詢失敗後加入 online、focus、回到前景自動重試及手動重新載入，不需切換頁面重新掛載。
- [x] 精簡私人歷史載入失敗文案，並調整手機版重新載入按鈕至訊息下方，避免遮住提示。
- [x] 實機驗證斷線時私人歷史載入失敗的 UI 文案與重新載入按鈕顯示正確。
- [x] 實機確認私人清單管理：離線查詢失敗後直接恢復連線可自動載入，手動重新載入正常，且只有查詢成功且確實為空時才顯示無紀錄。
- [x] 修正私人清單離線新增、重連同步後再刪除時偶發復活：相同旅程與帳號的快照寫入序列化，且雲端查詢期間若本機已變更，不允許舊查詢結果回蓋。
- [x] 實機完成 2 輪私人清單同步刪除測試：每輪離線新增 3 筆、恢復連線等待同步後刪除 3 筆，共 6 筆均未在刪除後立即復原。
- [x] 補充驗證私人清單刪除後切換頁面、重新登入及跨裝置確認，未發現已刪除項目延後復活。
- [x] 修正多行程於離線時私人清單管理提示不一致：不再以「是否有其他可複製來源」決定是否顯示離線狀態，並阻止離線時查詢 Supabase 歷史來源。
- [x] 實機確認在線依序開啟兩個行程、斷網後均可進入私人清單，且 Network 不新增私人歷史 Supabase 請求。
- [x] 修正離線時頁首仍顯示共用帳本同步中，以及共同／私人清單主畫面沿用 `syncing` 或模糊本機文案；離線狀態改為優先顯示完整同步說明。
- [x] 私人歷史來源改由各行程本機快取重建，並把有來源／無來源提示移至私人清單主畫面，不需進入管理模式。
- [ ] 實機重驗兩個行程：頁首顯示離線使用中；共同與私人清單主畫面顯示一致的離線同步說明；私人清單另依本機來源顯示正確歷史提示。
- [x] 修正未預載行程離線新增後覆蓋既有雲端私人清單：pending 保存首次修改基準，重連採三方合併；舊 pending 改採雲端＋本機聯集，避免資料遺失。
- [x] 恢復連線時重新查詢目前行程權限，使共同清單的雲端／本機同步文案不需切換行程即可更新。
- [ ] 阻擋發布驗證：登入後不進私人清單即離線，切到未預載且雲端已有資料的另一行程，離線新增 1 筆後重連；既有雲端項目與新項目須同時存在，其他裝置亦一致。
- [ ] 阻擋發布驗證：上述行程重連後，共同清單須自行切換為正確同步文案，不得要求切到另一行程再切回。
- [x] 共同／私人清單管理 UI 統一；歷史來源提示只在管理面板顯示，離線時兩者均不可複製。
- [x] 私人清單背景預載擴及所有具權限且出發日期為今天或未來的行程，歷史行程不背景預載。
- [x] 共同／私人清單刪除改為不確認，並於每次刪除後鎖定刪除操作 1 秒。
- [x] 修正私人清單 pending revision 清除與連續修改基準；同一帳號／行程的完整同步序列化，避免連續刪除及最後一筆復活。
- [ ] 發布前實機驗證：線上複製私人清單後逐筆刪至空清單，等待同步、切頁及重新登入後均不得復活。
- [ ] 發布前實機驗證：登入後不開啟私人清單，等待背景預載完成再斷網；今天與未來行程可顯示資料，歷史行程不應產生背景預載請求。

完整規格：

- `docs/16_V3.4.1_離線狀態與OtherInfo本機優先同步規格.md`

---

## V3.5.0 地點間預估移動資訊（需求已確認，尚未開始）

啟動條件：

- [x] V3.4.0 前端載入效能優化完成並通過驗證。
- [ ] V3.4.1 離線狀態與 Other Info 本機優先同步修正完成。

已確認範圍：

- [x] 只有直接相鄰且都有已確認地點的卡片才顯示交通資訊；無地點卡片會中斷關聯。
- [x] 每段獨立設定交通方式，預設開車，支援開車、步行及大眾運輸。
- [x] 卡片中間直接顯示交通圖示、預估時間及公里數。
- [x] 排除機場到機場的航班交通時間與距離。
- [x] 不計算停留時間、不手動輸入交通時間、不自動調整行程時間。
- [x] 行程間隔小於預估移動時間時顯示「行程間隔可能不足」警告。
- [x] Guest / User 只能查看快取；Trip Editor / Admin 才能查詢、切換及儲存。
- [x] App 不內建完整路線地圖，提供外部 Google Maps 按鈕。
- [x] 大眾運輸不細分工具、不列出多條路線；無班次時改用日間參考路線。
- [x] 採 Google Places、Google Routes 與 Supabase Edge Function，並以快取、配額及權限控制成本。
- [x] Routes API 每日查詢上限設為全專案合計 100 次；達限後保留既有快取，隔日恢復新查詢。

完整規格：

- `docs/15_V3.5.0_地點間預估移動資訊規格.md`

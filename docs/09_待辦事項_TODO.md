...
- [ ] V3.6.3：V3.6.2 發布後前端修正（範圍已彙整，尚未實作）。
  ...
  - [ ] 分離「客戶端目前是否必須更新」與「版本本身的發布政策」資料來源；更新提示維持既有安全阻擋，版本資訊／歷史顯示固定政策。
  - [ ] 補齊 V3.6.1 升級至 V3.6.2、V3.6.2 更新後版本資訊及歷史版本必要／一般更新標示測試。
  - [ ] 將交通方式替換為自託管 Material Symbols，保留文字、可存取名稱與既有版面；V3.7.0 直接沿用。
  - [ ] 將管理者「正在編輯歷史行程」提醒移出一般首頁，只在進入共用資料管理或開啟編輯旅程時顯示。
  - [ ] 將縮短天數改為兩個文案不同、需分別操作的確認階段；正式文案與模擬圖於實作前核准。
  - [ ] 補驗 iOS Safari／standalone PWA，並回歸桌面與 Android 操作。
  - [ ] 依最終修正是否涉及資料相容、安全、RLS 或同步風險，由 Product Owner 決定 V3.6.3 更新政策。
- [ ] V3.6.4：歷史唯讀參與者與跨裝置資料變動提醒（開發前須再次確認細節及風險）。
  - [x] 確認採「歷史唯讀參與者」名稱，不稱為 User；此為目前 Trip 的有效狀態，不變更全域角色。
  - [x] 確認保留完整歷史共用資料讀取能力、取消編輯者專用鎖定提醒，並禁止參與任何歷史共用資料編輯。
  - [ ] 定義參與者判定、角色解析、敏感資料、共用帳本、雲端換匯、私人資料與本機快取的一致行為。
  - [ ] 再次討論資料變動提醒的事件範圍、UI 文案、刷新粒度、未儲存狀態、離線／漏接備援及同裝置事件排除。

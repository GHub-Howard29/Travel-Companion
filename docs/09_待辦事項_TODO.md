...
- [ ] V3.9.4：完成 Commons 精確搜尋零候選自動續頁、session 12／30 秒／40 檔累積上限、12 張精確候選、一層相關分類延伸候選與搜尋範圍純狀態提示的本機互動驗證；實作自行上傳的拍照／原生選檔入口、紅色侵權提醒，以及 76×76 預覽、完整原圖 `100%`、最大 `250%`、同圖模糊背景與 640×640 WebP 輸出。Pexels 不接入，Pixabay 永久取消。
- [ ] V3.9.5：完成航班卡片排除下一站地面交通，以及每日到達／離開／時間調整四碼、全形、IME 輸入規格的本機驗證；詳見《57》。

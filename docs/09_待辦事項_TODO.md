...
- [ ] V3.9.1 第一優先：Commons 實體導向候選照片精準篩選規格已核准並取得分階段實作授權；精確授權／來源／尺寸／強證據硬篩、25 分門檻、可解釋評分、固定同分排序、`needs-review`、互斥終態，以及 9 次／20 秒／30 張上限已完成無網路純邏輯與自動契約測試，尚未接入 Edge Function 正式流量。P18／P373／Category／結構化證據取得、唯一 QID 解析、短效 token／共享快取與完整 UI 尚未實作。影像辨識維持排除；migration 細節與真實矩陣／spike 仍須個別核准。詳見《50》。

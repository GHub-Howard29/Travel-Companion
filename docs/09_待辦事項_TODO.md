...
- [ ] V3.6.1：Android 記帳金額欄位與 PWA 更新流程修正（已進入手動發布程序，等待完成回報）。
  ...
  - [ ] 以 V3.6.0 舊版快取升級至 V3.6.1，驗證 Android／iOS 手機與桌面瀏覽器／PWA 都只需點擊一次，且提示自動消失、頁面自動載入新版，不發生空白頁或重複重載。
...
- [ ] V3.6.2：行程天數縮短刪除與歷史行程鎖定（畫面與範圍已確認，待開發）。
  ...
  - [ ] 實作縮短影響統計、可捲動的多 Day 二次確認，以及確認後一次刪除所有超出天數的 `daysData`。
  - [ ] 歷史行程的 `trip_editor` 停用所有共用資料管理與編輯入口並顯示完整唯讀說明；仍可查看完整資料。
  - [ ] 在 Supabase 的 Trip、共同清單、共同帳本、其他資訊與相關共用寫入政策，以台灣日期阻擋歷史行程 `trip_editor` 寫入；`super_admin` 保留原有權限。
  - [ ] 覆驗縮短 1 天、縮短多天、移除空白 Day、移除有卡片／路線 Day、取消確認、增加天數、歷史角色矩陣、離線與同步失敗。
  - [ ] 通過 lint、build、資料庫驗證、桌面與 Android／iOS PWA 回歸。

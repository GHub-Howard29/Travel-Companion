...
- [x] V3.6.2：行程天數縮短刪除與歷史行程鎖定（開發、migration、驗證、Git 版本標籤與手動部署已完成；發布狀態為「已發布」）。
  ...
  - [ ] BUG029 已轉列 V3.6.3：更新完成後的版本資訊把 V3.6.2 錯誤顯示為「一般更新」；等 V3.6.2 實際驗證完成後與同批問題一併處理。
  - [ ] 完成發布後桌面、Android／iOS PWA 實機矩陣並記錄結果。
- [ ] V3.6.3：V3.6.2 發布後驗證修正（等待 V3.6.2 實際驗證完成後進入實作）。
  - [x] 納入 BUG029 初步分析：`VersionInfoModal` 把 `isMandatoryForCurrentClient` 同時當成版本發布政策，導致升級完成後錯顯示「一般更新」。
  - [ ] 彙整 V3.6.2 縮短天數、歷史角色、離線／同步及桌面、Android／iOS PWA 實測結果，凍結 V3.6.3 最終修正範圍。
  - [ ] 分離「客戶端目前是否必須更新」與「版本本身的發布政策」資料來源；更新提示維持既有安全阻擋，版本資訊／歷史顯示固定政策。
  - [ ] 補齊 V3.6.1 升級至 V3.6.2、V3.6.2 更新後版本資訊及歷史版本必要／一般更新標示測試。
  - [ ] 將 V3.6.2 實測發現的其他問題與 BUG029 一併修正、回歸及記錄；未完成實測前不提前改程式。

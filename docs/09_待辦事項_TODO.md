...
- [ ] V3.9.1 第一優先：Commons 實體導向候選照片精準篩選規格已核准並取得分階段實作授權；P18／P373／Category／結構化證據、唯一 QID、硬篩、可解釋分數、`needs-review`、停止狀態、請求預算、短效 token／快取與最多 30 張契約尚未實作。影像辨識維持排除；migration 細節與真實矩陣／spike 仍須個別核准。詳見《50》。

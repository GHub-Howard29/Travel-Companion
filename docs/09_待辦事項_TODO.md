...
- [ ] V3.9.1 第一優先：Commons 實體導向候選照片精準篩選規格已核准並取得分階段實作授權；精確硬篩／評分／排序／終態純邏輯、Wikidata／Commons 離線回應解析與順序作業協調器、第五批固定請求參數／中介資料清理／候選證據組裝、第六批短效 AES-GCM 續頁 token／固定 `contractVersion` 回應契約、第七批 transport-neutral request plan／允許上游 URL 白名單／429／503／timeout 錯誤分類、第八批公開候選回應投影／內部證據遮罩／不透明 token 邊界、第九批專案級每分鐘／每日用量閘門與 25 秒單一上游鎖純函式契約、第十批共享快取 key／TTL／容量／敏感欄位排除純函式契約、第十一批用量統計 delta／終態／延遲分類純函式契約、第十二批日彙總日期／保留／合併／月份加總純函式契約，以及第十三批日彙總持久化邊界／敏感欄位排除／當日原子更新輸入純函式契約已完成自動契約測試，尚未接入 Edge Function 正式流量。協調器已限制單一進行中請求、3 秒單次 timeout、9 次／20 秒／30 張硬停與 continuation 同層優先；文字搜尋只接受單一已採用詞，直接 Category 不遞迴，P18／多個直接 Category／P180 證據可合併後交由既有硬篩與排序。真實上游取得、短效 token 金鑰部署、共享快取 migration、用量 migration 與完整 UI 尚未實作。影像辨識維持排除；migration 細節與真實矩陣／spike 仍須個別核准。詳見《50》。

...
- [ ] V3.9.1 跨日複製四碼時間輸入修正：抵達／離開欄位接受有效四碼數字並自動補成 `HH:MM`，保留既有冒號、必填、時分與先後驗證；解決行動裝置數字鍵盤無法輸入冒號。桌面與 390×844 UI mock、提交警告跳窗與無障礙行為已核准；無 schema／API 範圍，仍待實作授權。詳見《51》。
- [x] V3.9.1 第一優先：Commons 實體導向候選照片精準篩選。2026-09-14 已核准 P18／P373／Category／結構化證據順序、唯一 QID 解析與歧義停止、硬篩、可解釋分數、`needs-review`、`no-suitable-image`、桃園第一航廈驗收與單一 Wikimedia 順序作業例外，以及每次最多 9 次／20 秒、實體證據 7 天與候選／無適圖 10 分鐘快取；AI 候選詞須人工逐一採用，禁止自動多語／別名搜尋；每 session 最多首批加 4 次換批、最多 30 張，達限回傳 `inspection-limit-reached`；不建立不適合標記，認為不適合只換一批或改搜尋詞；選圖、確認、裁切、放大檢視與無障礙 UI 已核准。影像辨識與程式實作仍待各別核准；詳見《50》。
...
- [x] V3.9.1 Commons 搜尋提示修正：2026-09-15 已核准 `empty-first-page`「找不到符合條件的照片，請調整搜尋詞後再試。」、`duplicate-page`「這一批沒有新的照片；可再換一批或調整搜尋詞。」、`exhausted`「已沒有更多照片；可調整搜尋詞或改用其他來源。」。三者不自動切換來源，僅 `duplicate-page` 可再換一批；去重範圍及重置時機沿用《49》5.2。UI 行為已核准，實作仍待整案實作授權。
